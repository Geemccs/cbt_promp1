...
<div class="mb-4 flex items-center justify-between">
    ...
    <a href="{{ url()->previous() }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm px-4 py-2 rounded-lg">
            ...
            <tbody>
                ...
                <tr class="border-b hover:bg-gray-50">
                    ...
                    <td class="px-3 py-2 text-center">
                        <span class="{{ $us->jumlah_keluar > 0 ? 'text-red-600 font-bold' : 'text-gray-400' }}">
                            {{ $us->jumlah_keluar ?? 0 }}
                        </span>
                    </td>
                    ...
                    <td class="px-3 py-2">
                        @if($us->status !== 'belum')
                        <form action="{{ route('admin.ruang-ujian.reset-siswa', $ruangUjian->id) }}" method="POST" class="inline"
                            onsubmit="return confirm('Reset ujian siswa ini? Semua jawaban akan dihapus.')">
                            @csrf
                            <input type="hidden" name="siswa_id" value="{{ $us->siswa_id }}">
                            <button type="submit" class="text-xs text-red-500 hover:text-red-700 border border-red-200 px-2 py-1 rounded">
                ...
                @empty
                <tr><td colspan="8" class="text-center py-6 text-gray-400">Belum ada peserta.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
