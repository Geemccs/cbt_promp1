...
<div class="bg-white rounded-xl shadow p-6">
    <div class="flex items-center justify-between mb-4">
        <h2 class="text-lg font-semibold text-gray-700">Daftar Bank Soal</h2>
        ...
        <table id="tabelBankSoal" class="w-full text-sm">
            ...
            <tbody>
                ...
                <tr class="border-b hover:bg-gray-50">
                    ...
                    <td class="px-3 py-2">{{ $bs->guru?->name ?? 'Admin' }}</td>
                    <td class="px-3 py-2 text-center">{{ $bs->waktu_mengerjakan }}</td>
                    <td class="px-3 py-2 space-x-1">
                        <a href="{{ route('admin.bank-soal.edit', $bs->id) }}"
                            class="text-blue-600 hover:text-blue-700 text-xs bg-blue-50 px-2 py-1 rounded" title="Edit Soal">
                            <i class="fas fa-pencil-alt mr-1"></i>Soal
                        </a>
                        <button onclick="openEdit({{ $bs->id }},'{{ addslashes($bs->nama_soal) }}',{{ $bs->mapel_id }},{{ $bs->guru_id ?? 'null' }},{{ $bs->waktu_mengerjakan }},{{ $bs->bobot_pg }},{{ $bs->bobot_essay }},{{ $bs->bobot_menjodohkan }},{{ $bs->bobot_benar_salah }})"
                            class="text-yellow-600 hover:text-yellow-700"><i class="fas fa-cog"></i></button>
                        <form action="{{ route('admin.bank-soal.destroy', $bs->id) }}" method="POST" class="inline"
                            onsubmit="return confirm('Hapus bank soal ini? Semua soal di dalamnya juga akan terhapus.')">
                            @csrf @method('DELETE')
                ...
                @empty
                <tr><td colspan="6" class="text-center py-6 text-gray-400">Belum ada bank soal.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
        ...
        <form action="{{ route('admin.bank-soal.store') }}" method="POST">
            ...
            <div class="bg-gray-50 rounded-lg p-4 mb-4">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs text-gray-600 mb-1">PG (%)</label>
                        <input type="number" name="bobot_pg" value="100" min="0" max="100" step="0.01" required class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                    </div>
                    <div>
                        <label class="block text-xs text-gray-600 mb-1">Essay (%)</label>
                        <input type="number" name="bobot_essay" value="0" min="0" max="100" step="0.01" required class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                    </div>
                    <div>
                        <label class="block text-xs text-gray-600 mb-1">Menjodohkan (%)</label>
                        <input type="number" name="bobot_menjodohkan" value="0" min="0" max="100" step="0.01" required class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                    </div>
                    <div>
                        <label class="block text-xs text-gray-600 mb-1">Benar/Salah (%)</label>
                        <input type="number" name="bobot_benar_salah" value="0" min="0" max="100" step="0.01" required class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                    </div>
                </div>
            </div>
...
<div id="modalEdit" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-lg p-6 max-h-screen overflow-y-auto">
        ...
        <form id="formEdit" method="POST">
            ...
            <div class="mb-3">
                <label class="block text-sm font-medium text-gray-700 mb-1">Nama Bank Soal</label>
                <input type="text" name="nama_soal" id="editNama" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
            </div>
            <div class="mb-3">
                <label class="block text-sm font-medium text-gray-700 mb-1">Mata Pelajaran</label>
                <select name="mapel_id" id="editMapel" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                    @foreach($mapels as $m)<option value="{{ $m->id }}">{{ $m->nama_mapel }}</option>@endforeach
                </select>
            </div>
            <div class="mb-3">
                <label class="block text-sm font-medium text-gray-700 mb-1">Waktu Mengerjakan (menit)</label>
                <input type="number" name="waktu_mengerjakan" id="editWaktu" min="1" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
            </div>
            <div class="bg-gray-50 rounded-lg p-4 mb-4">
...
function openEdit(id,nama,mapelId,guruId,waktu,pg,essay,menjodohkan,benarSalah){
    document.getElementById('formEdit').action='/admin/bank-soal/'+id;
